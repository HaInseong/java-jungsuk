 ...
 * ★ 상속의 조상 자손관계에서만 가능하다.
 * 형변환 가능 여부는 ★생성한 실제 객체를 기준★으로 판단한다.
...
* = ★ 인스턴스(생성한 실제 객체)의 원래 기능을 모두 사용하기 위해서
* 1. 조상인 Car타입으로 매개변수로 설정하여 자손들을 모두 받을 수 있게 열어두고,
* 2. instance of 연산자로 들어온 매개 변수의 타입을 확인하고 타입에 맞게 형변환 시킨다.
* 3. 그럼 모든 자손을 매개변수로 받을 수 있는 동시에 전달 받은 매개변수의 원래 기능을 사용할 수 있다.
* 이게 바로 다형성이다.*/
...
//        car.water(); //Error = Car타입 리모컨에는 water 기능이 없다.
        fe2 = (FireEngine) car;
        fe2.water(); //★ car 참조변수가 가르키던 실제로 생성된 객체는 원래부터 FireEngine이었다.
//        Ambulance a = (Ambulance)f; //★상속관계가 아닌 클래스 간의 형변환 불가★


        //★ 리모컨은 FireEngine이고 실제 생성하고 가르키는 객체는 Car() 객체라면
        //사용 가능한 기능보다 리모컨의 갯수가 많다. 그러므로 오류가 발생한다.
        //실제 있는 기능보다 리모컨 버튼 수가 더 많을 순 없음 ex) 서비스센터 전화 불남
        Car c = new Car(); //멤버 갯수 4개
        //멤버 갯수 5개 <- 멤버 갯수 4개
        FireEngine fe3 = (FireEngine) c; //★ Error 발생 - 자손 타입의 참조변수로 조상 타입의 객체를 가리킬 수 없다.
    }
}
